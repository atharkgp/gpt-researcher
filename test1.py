        ...
        for s in splits:
            if self._length_function(s) < self._chunk_size:
                ...
                if not new_separators:
                    l = len(s)
                    n = math.floor(l / self._chunk_size) + 1
                    new_size = l // n

                    i = 0
                    while i < l:
                        # Find the approximate end of the current chunk
                        end = min(i + new_size, l)
                        
                        # Adjust the end to avoid splitting a word
                        if end < l:
                            while end > i and not s[end:end+2] == ". ":
                                end -= 1
                        
                        # If we couldn't find a space, extend to the next space (rare edge case)
                        if end == i:
                            end = min(i + new_size, l)
                            while end < l and not s[end:end+2] == ". ":
                                end += 1

                        # Add the chunk and update the index
                        final_chunks.append(s[i:end].strip())
                        i = end
                else:
